(() => {
  ...
  const ctaWords = [...document.querySelectorAll('[data-cta-rotator] span')];
  if (ctaWords.length && !reduceMotion) {
    let ctaIndex = 0;
    setInterval(() => {
      const prev = ctaIndex;
      ctaIndex = (ctaIndex + 1) % ctaWords.length;
      ctaWords[prev].classList.remove('is-active');
      ctaWords[prev].classList.add('is-leaving');
      ctaWords[ctaIndex].classList.remove('is-leaving');
      ctaWords[ctaIndex].classList.add('is-active');
      setTimeout(() => ctaWords[prev].classList.remove('is-leaving'), 500);
    }, 2000);
  }
